...
#include "dds_demoTypeSupportImpl.h"
...
namespace demo {
::DDS::DataWriter_ptr DataTypeTypeSupportImpl::create_datawriter()
{
  typedef OpenDDS::DCPS::DataWriterImpl_T<DataType> DataWriterImplType;
...
::DDS::DataReader_ptr DataTypeTypeSupportImpl::create_datareader()
{
  typedef OpenDDS::DCPS::DataReaderImpl_T<DataType> DataReaderImplType;
...
::DDS::DataReader_ptr DataTypeTypeSupportImpl::create_multitopic_datareader()
{
  typedef OpenDDS::DCPS::DataReaderImpl_T<DataType> DataReaderImplType;
  typedef OpenDDS::DCPS::MultiTopicDataReader_T<DataType, DataReaderImplType> MultiTopicDataReaderImplType;
...
const OpenDDS::DCPS::MetaStruct& DataTypeTypeSupportImpl::getMetaStructForType()
{
  return OpenDDS::DCPS::getMetaStruct<DataType>();
}
#endif /* !OPENDDS_NO_CONTENT_SUBSCRIPTION_PROFILE */

bool DataTypeTypeSupportImpl::has_dcps_key()
{
  return TraitsType::gen_has_key ();
}

const char* DataTypeTypeSupportImpl::default_type_name() const
{
  return TraitsType::type_name();
}

DataTypeTypeSupport::_ptr_type DataTypeTypeSupportImpl::_narrow(CORBA::Object_ptr obj)
{
...
template<>
struct MetaStructImpl<demo::DataType> : MetaStruct {
  typedef demo::DataType T;
  ...
  {
    const demo::DataType& typed = *static_cast<const demo::DataType*>(stru);
    ...
    throw std::runtime_error("Field " + OPENDDS_STRING(field) + " not found or its type is not supported (in struct demo::DataType)");
  }
  ...
  Value getValue(Serializer& ser, const char* field) const
  {
    ...
    throw std::runtime_error("Field " + OPENDDS_STRING(field) + " not valid for struct demo::DataType");
  }
  ...
  {
    static const char* names[] = {"Handle", "Timestamp", 0};
    return names;
  }
    ...
    const char* rhsFieldSpec, const MetaStruct& rhsMeta) const
  {
    ...
    if (std::strcmp(field, "Handle") == 0) {
      static_cast<T*>(lhs)->Handle = *static_cast<const CORBA::Long*>(rhsMeta.getRawField(rhs, rhsFieldSpec));
      return;
    }
    if (std::strcmp(field, "Timestamp") == 0) {
      static_cast<T*>(lhs)->Timestamp = *static_cast<const CORBA::Double*>(rhsMeta.getRawField(rhs, rhsFieldSpec));
      return;
    }
    throw std::runtime_error("Field " + OPENDDS_STRING(field) + " not found or its type is not supported (in struct demo::DataType)");
  }
...
{
  static MetaStructImpl<demo::DataType> msi;
  return msi;
}
...
{
  ACE_UNUSED_ARG(ser);
  MetaStructImpl<demo::DataType>().getValue(ser, "");
  return true;
}
